...
$$ att_t = \sum_{k=1}^3 \pi^{att}_{t,k} ~ \text{nct}(\mu^{att}_k, \tau^{att}_k, \nu), ~ ~  ~ ~  ~ ~ ~ ~ ~ ~ ~ ~ def_t = \sum_{k=1}^3 \pi^{def}_{t,k} ~ \text{nct}(\mu^{def}_k, \tau^{def}_k, \nu). $$

where $\mu^{att}_k, \tau^{att}_k, \mu^{def}_k, \tau^{def}_k$ for $k=1,2,3$ are the population mean and precision of the attack and defense effects of the three team categories.

### Model implementation and results

Once introduced the models theoritecally, we need to implement them, specifying a prior disttibution for the parameters, fitting on the Serie A 1991-92 data and compare the results with the ones of Baio, G., Blangiardo, M. (2010).
...
As suggested in Baio, G., Blangiardo, M. (2010), the prior distributions for the hyper-parameters and the home effect are modelled indipendently by using non-informative prior distributions:
