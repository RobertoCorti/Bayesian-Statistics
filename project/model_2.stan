...
model {
  ...
  att[3] ~ normal(2, 1000) T[0,3];
  def[3] ~ normal(-2, 1000) T[-3,0];
  ...
  for(t in 1:T){
    real lp[3];
    lp[1]= log(pi_att[t,1]) + student_t_lpdf( att_team_star[t] | 4, att[1], sigma_att[1]);
    lp[2]= log(pi_att[t,2]) + student_t_lpdf( att_team_star[t] | 4, att[2], sigma_att[2]);
    lp[3]= log(pi_att[t,3]) + student_t_lpdf( att_team_star[t] | 4, att[3], sigma_att[3]);
    target += log_sum_exp(lp);
    
    lp[1]= log(pi_def[t,1]) + student_t_lpdf( def_team_star[t] | 4, def[1], sigma_def[1]);
    lp[2]= log(pi_def[t,2]) + student_t_lpdf( def_team_star[t] | 4, def[2], sigma_def[2]);
    lp[3]= log(pi_def[t,3]) + student_t_lpdf( def_team_star[t] | 4, def[3], sigma_def[3]);
    target += log_sum_exp(lp);
  }
